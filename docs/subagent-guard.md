...
Every other tracked Claude hook entry in `.claude/settings.json` carries the Grok marker guard whose inventory and rationale docs/turnend-guard.md "Harness integrations" owns, so Grok's Claude-compatible settings loading cannot run a second copy of an event Grok already covers through its own `.grok/hooks/` registration.
