...
A captain-message turn never runs the drain, so `bin/fm-voice-pending.sh`, the Claude primary's tracked `UserPromptSubmit` hook, presents the same pending voice rows under the same `VOICE` heading at the start of that turn; it is read-only and presentation-only, taking no lock and writing nothing, and the drain with its generation-bound acknowledgement remains the only mutation and acknowledgement path (`tests/fm-voice-pending.test.sh`).
