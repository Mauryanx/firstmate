...
The reordering only presents: answering a spoken turn is the `answer-voice-turn` skill's accept-and-publish sequence, and the drain's own generation-bound acknowledgement remains the only thing that retires the row.
That acknowledgement retires a voice row only once its request has left `saved`, which the transport records by moving `state/inbox/vc-<id>.note` to `handled/` at acceptance or rejection; a row whose note is still pending is held in the queue with its claim, reported on stderr, and presented again under `VOICE` by the next drain, while every other row through the cutoff is retired exactly as before.
`tests/fm-wake-drain-voice-hold.test.sh` pins that against the real transport and drain: a still-saved voice row survives the acknowledgement that retires an ordinary row beside it, and is retired once the request is accepted.
