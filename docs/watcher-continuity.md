...
Within that view, a captain voice note - a `check` row keyed `inbox:vc-<id>`, written by the voice conversation transport beside its durable record `state/inbox/vc-<id>.note` - is presented ahead of every other row under a `VOICE` heading, with the remaining rows labelled as coming after it, while every sequence number, claim, deduplication, and acknowledgement cutoff stays exactly what it would be without the reordering.
`tests/fm-wake-drain-voice-first.test.sh` pins that precedence in both enqueue orders against the real drain, together with the unchanged acknowledgement contract and a typed note's ordinary presentation.
A captain-message turn never runs the drain on its own, so `bin/fm-voice-pending.sh`, the Claude primary's tracked `UserPromptSubmit` hook, presents the same pending voice rows under the same `VOICE` heading at the start of that turn; it is read-only and presentation-only, taking no lock and writing nothing, and it stands down silently unless the session it fires in owns the home's fleet lock, so a second session in the same home is never told to answer a note the lock owner is handling (`tests/fm-voice-pending.test.sh`).
The drain with its generation-bound acknowledgement remains the only mutation and acknowledgement path: a captain-message turn that receives `VOICE` rows from the hook then runs `bin/fm-wake-drain.sh`, which presents those rows again under `VOICE` with the rest of the queue behind them, handles them, and runs the exact `WAKE_ACK_REQUIRED` command the drain printed, because answering the note and retiring it with `bin/fm-inbox.sh drain --ack <id>` leaves its wake row queued, and an unacknowledged row is presented again by the hook on every later captain-message turn and by the next drain.
Other primary harnesses present voice notes at session start and at wake-handling turns through the drain, because this change wires no turn-start hook point for them.
