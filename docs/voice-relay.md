...
On a substantive turn it files the captain's committed transcript through the existing conversation transport, exactly as the browser pilot does, and says nothing at all.
The line he hears while Firstmate reads is generated by the agent platform itself, in the context of what he just said, with a prompt override forbidding any claim of progress or result.
That is the feature this bridge used to manufacture, badly: a subject drawn out of his own words by rule, hedged by a gate that had to keep narrowing until it refused most ordinary phrasings, spoken over a platform line that had not been switched on.
It is switched on now and it is better than what was here, so the manufactured opener is gone, and with it the held turn that existed only to let that opener flow into the answer as one utterance.
The bridge speaks Firstmate's published words and nothing else.

Every turn still has to finish inside the agent's cascade timeout: measured against the live platform, a turn still open when that expires ends the whole conversation with a cascade error and drops the captain mid-call.
Filing his words is bounded by what that leaves rather than by the transport's own longer allowance, or one slow call would carry the turn past the timeout.
A turn that runs out of budget refuses, audibly, rather than reassuring him about a request that was never filed; refusing also leaves the turn unhandled, so a retry can still file what he said.
The margin held back is deliberately wide: an agent on another of the vendor's products was killed fourteen seconds in, on a turn that had already finalised around eleven, so the real ceiling sits lower than the timeout suggests and ending before it is not by itself safe.
...
Below that floor the reply is left waiting instead, which keeps it durable and auditable.
...
Turn the platform's generated pause line on, with a prompt override forbidding any claim of progress or result.
