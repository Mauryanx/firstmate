...
A turn that arrives bound to a question this section asked, one that named the missing part of an incomplete order, completes that order, and the pair is the order.
The bound turn supplies the missing part alone by design, so it is never read on its own and never draws a second question for a part the pair already supplies.
The incomplete order is read from the record rather than from memory, so the pair survives a session restart: `audit` lists every request in capture order with its `turn_id`, the `previous_turn_id` naming the turn it follows, and its `note_id`, and the note of a request already accepted is under `state/inbox/handled/`, read as step 2 of [`../SKILL.md`](../SKILL.md) describes.
...
Record what such an answer settles through the ordinary decision path rather than treating it as a dispatch.
