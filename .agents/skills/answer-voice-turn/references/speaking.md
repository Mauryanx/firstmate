...
The first is a missing-part question this section asked, and its reply is read for one thing: whether it supplies the part that question named.
When it does, it completes that order, the pair is the order, and the work starts as soon as the pair is an order as defined above, with no second question published for a part the pair already supplies.
...
The second is the `-say-again` question the reconcile route in [`publication.md`](publication.md) publishes for a turn that was lost.
