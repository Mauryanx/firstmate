...
The order a bound reply was asked about is named by that reply's own `question_binding`, which was derived from the order's `request_id`, and the order's words are read from the record rather than from memory so the pair survives a session restart: `audit` lists that request with its `note_id`, and the note of a request already accepted is under `state/inbox/handled/`, read as step 2 of [`../SKILL.md`](../SKILL.md) describes.
