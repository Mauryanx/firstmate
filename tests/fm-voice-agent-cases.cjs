 ...
 try {
  ...
  await page.route('**/elevenlabs.js', route => route.fulfill({contentType:'text/javascript', body:`
    ...
    window.ElevenLabsClient = {Conversation: {startSession: async opts => {
      ...
      // The page has no route that claims a reply, deliberately: adding one to
      // the product for a test's sake would put a claim surface into a system
      // whose whole honesty property is that only Firstmate's words are spoken.
      // So the harness claims through the owner transport instead, and this
      // fixture WAITS for the claim it did not cause. That is the shape the real
      // system produces - the bridge claims, the platform then speaks - and
      // waiting for it is what makes the page's claim-anchored predicate get
      // genuinely claimed state rather than state this fixture invented.
      const claim = async id => {
        for (let waited = 0; waited < 30000; waited += 100) {
          const state = await (await fetch('/poll', {method:'POST', headers:{'Content-Type':'application/json'},
                                                     body:'{}'})).json();
          const reply = (state.replies || []).find(r => r.response_id === id);
          if (reply && reply.delivery && reply.delivery.state !== 'waiting') return;
          await wait(100);
        }
        throw Error('the harness never claimed ' + id);
      };
