...
# set to a primary-shaped checkout and a Claude-shaped payload on stdin. The
# incident this pins: on 2026-09-10 a captain text message started a turn that
# never ran the drain, so eleven queued spoken turns were never seen. The
# presenter must print the same VOICE section the drain prints, and it must
# change nothing: the queue stays byte-identical and state/ gains no file.
set -u
...
install_presenter() {  # <dir>
  local script
  mkdir -p "$1/bin"
  for script in fm-voice-pending.sh fm-primary-scope-lib.sh fm-hook-host-lib.sh fm-wake-lib.sh; do
...
# Run the tracked UserPromptSubmit command the way Claude Code would.
...
run_hook() {  # <project-dir> <state> <payload> <stdout-file> <stderr-file>
  local cmd
  ...
  printf '%s' "$3" | env -u FM_ROOT_OVERRIDE -u FM_HOME -u GROK_AGENT -u GROK_HOOK_EVENT \
    CLAUDE_PROJECT_DIR="$1" FM_STATE_OVERRIDE="$2" sh -c "$cmd" > "$4" 2> "$5"
}

# Every file under <state> with its content hash, so a test can prove the hook
# neither created nor modified anything there, the recovery marker included.
...
test_voice_note_behind_ordinary_wake_is_presented_alone_and_read_only() {
  ...
  [ "$(state_manifest "$state")" = "$before_manifest" ] \
    || fail "primary-voice: the hook created or changed a file under state/ (a claim, the recovery marker, or a lock): $(ls -A "$state")"
