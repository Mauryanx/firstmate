...
state/               runtime records and signals; gitignored
  ...
  inbox/             captain notes captured out of band by bin/fm-inbox.sh, including the voice handover's queued requests; each note appends one `check` wake and, except a `vc-` conversation note (next entry), stays pending until acknowledged with `bin/fm-inbox.sh drain --ack <id>`, which moves it to inbox/handled/ (docs/voice-relay.md)
  voice-conversation/  durable journal, publication policy and lock for a live voice conversation; its `vc-` request notes live in inbox/ above and are answered, never drained, through `answer-voice-turn`
...
3. **Wake queue** - when locked, drains and presents the durable wake queue without running the inactive-outcome scan inline, and prints the raw records prominently as this turn's first work queue; a clearly labeled status-event annotation may follow a valid `signal` record and includes every status line still unread at the presentation cursor, but never replaces the raw record or current-state reconciliation, and a lapsed watcher chain still surfaces here via the same guard alarm.
   A captain voice note's `check` record is presented ahead of every other record under a `VOICE` heading and is answered first (`docs/watcher-continuity.md`).
...
- `answer-voice-turn` - load on any `check:` wake whose key is `inbox:vc-*`, and before accepting, publishing, or rejecting a reply on a voice conversation.
